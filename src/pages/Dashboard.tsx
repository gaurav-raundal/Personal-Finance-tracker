...
import { BarChart, BarList, PieChart } from "@/components/ui/chart";
...
import { ArrowDownCircle, ArrowUpCircle, IndianRupee, Plus } from "lucide-react";
...
const Dashboard = () => {
  const { user } = useAuth();
  const { addTransaction, getRecentTransactions, getMonthlyData, getDailyData, getCategoryData } = useTransactions();
  ...
  const categoryData = getCategoryData().filter(item => item.type === categoryView);
          ...
          <CardContent>
            {categoryData.length > 0 ? (
              ...
              <div className="py-16 text-center text-muted-foreground">
                <p>No {categoryView} data to display</p>
                <Button
                  variant="link"
                  onClick={() => setIsDialogOpen(true)}
                  className="mt-2"
